...
public class ProjectSetup {
    ...
    static void readInstallerEnvVariables(String[] args) {
        ...
        if (args.length == 0){
            System.out.println("Usage: java -cp ProjectSetup.jar ProjectSetup [-t | --dbms]=<database platform> [-h | --host]=<hostname> [-n | port]=<port number> [-u | --username]=<username> [-p | --password]=<password> [-s | --service]=<Service name (for Oracle)> [-d | --database]=<Database name> [-o | --project]=<Project name>");
            System.exit(0);
        }
