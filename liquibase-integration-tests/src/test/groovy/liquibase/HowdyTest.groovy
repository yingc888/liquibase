 ...
 */
public class HowdyTest extends Specification {
  ...
  def "test postgres"() {
      given:
      //database = DatabaseFactory.getInstance().getDatabase("postgresql")
    ...
    public construct() throws Exception {
        construct("pgsql", DatabaseFactory.getInstance().getDatabase("postgresql"));
    }

    protected construct(String changelogDir, Database dbms) throws Exception {
    ...
    protected Liquibase createLiquibase(String changeLogFile, ResourceAccessor resourceAccessor) {
        ExecutorService.getInstance().clearExecutor(database);
    ...
    public void testEncodingUpdating2SQL() throws Exception {
        ...
        assertTrue("Update to SQL preserves encoding",
            new RegexMatcher(writer.toString(), [
